...
export default function Sidebar() {
  return (
    <aside className="w-full md:w-64 shrink-0 md:sticky md:top-20 flex flex-col items-center gap-0">

      {/* Profile photo — sits above the card */}
      ...
      <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl pt-14 pb-6 px-5 flex flex-col items-center gap-4 text-center">
        ...
        {/* Quick info */}
        <div className="w-full border-t border-slate-200 pt-3 space-y-2 text-left">
          {INFO_ITEMS.map((item, i) => (
            <div key={i} className="flex items-start gap-2 text-xs text-slate-500">
              <span className="leading-tight mt-0.5 shrink-0">{item.icon}</span>
        ...
        {/* Social icons */}
        <div className="w-full border-t border-slate-200 pt-3 flex items-center justify-center flex-wrap gap-2">
          {SOCIALS.map(({ href, label, icon, external = true }) => (
            <a
              ...
              aria-label={label}
              className="w-8 h-8 rounded-full bg-white border border-slate-200 hover:bg-blue-600 hover:border-blue-600 hover:text-white flex items-center justify-center text-slate-500 transition-all duration-200 hover:scale-110 shadow-sm"
            >
              {icon}
            </a>
          ))}
